...
import com.example.g9ems.data.LoginSession
import com.example.g9ems.data.User
import com.example.g9ems.data.UserRole
...
class DatabaseManager private constructor(context: Context) {
    ...
    fun authenticateUser(medicNumber: String, password: String): User? {
        try {
            ...
            if (allResults.isEmpty()) {
                Log.d("EMS-DB", "❌ No user found with medic number: $medicNumber")
                return null
            }
            ...
            if (storedPassword != hashedInput) {
                Log.d("EMS-DB", "❌ Invalid password for user: $medicNumber")
                return null
            }
            ...
            Log.d("EMS-DB", "✅ User authenticated: ${user.firstName} ${user.lastName}")
            ...
            Log.e("EMS-DB", "❌ Authentication error", e)
            return null
        }
    }
    ...
    fun createUser(user: User, password: String): Boolean {
        try {
            ...
            if (query.execute().allResults().isNotEmpty()) {
                Log.d("EMS-DB", "❌ User already exists: ${user.medicNumber}")
                return false
            }
            ...
            collection.save(doc)
            Log.d("EMS-DB", "✅ User created: ${user.medicNumber}")
            return true

        } catch (e: Exception) {
            Log.e("EMS-DB", "❌ Error creating user", e)
            return false
        }
    }
    ...
    fun closeDatabase() {
        try {
            ...
            Log.e("EMS-DB", "Error closing database", e)
        }
    }
